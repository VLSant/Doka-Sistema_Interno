...
if ($null -eq $Options) {
    try {
        $rawConfig = Get-Content -LiteralPath $ExtConfig -Raw
        $contextFile = $null
        $markerStart = $null
        $markerEnd = $null
        foreach ($line in ($rawConfig -split "`n")) {
            $trimmed = $line.Trim()
            if ($trimmed -match '^context_file:\s*(.+)$') {
                $contextFile = $Matches[1].Trim().Trim('"').Trim("'")
            } elseif ($trimmed -match '^start:\s*(.+)$') {
                $markerStart = $Matches[1].Trim().Trim('"').Trim("'")
            } elseif ($trimmed -match '^end:\s*(.+)$') {
                $markerEnd = $Matches[1].Trim().Trim('"').Trim("'")
            }
        }
        if ($contextFile) {
...
if ($PlanPath) {
    if ([System.IO.Path]::IsPathRooted($PlanPath)) {
        $PlanPath = [System.IO.Path]::GetRelativePath($ProjectRoot, $PlanPath)
    }
