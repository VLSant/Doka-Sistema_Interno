...
- **FR-020A**: A processed lot with official status `importado` MAY update present mirror records and MUST mark absent previously mirrored records as `removido` when it is complete for the posto/data.
- **FR-020B**: A processed lot with official status `importado_com_alertas` MAY update present mirror records and MUST mark absent previously mirrored records as `removido` when alerts are non-blocking and the lot is complete for the posto/data.
